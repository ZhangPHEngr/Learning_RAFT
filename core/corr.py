...
class CorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4):
        ...
        self.corr_pyramid.append(corr)  # C0 C1 C2 C3
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            centroid_lvl = coords.reshape(batch * h1 * w1, 1, 1, 2) / 2 ** i   # (b,h,w,2) -> (bhw,1,1,2) 某一尺度下的当前点坐标
        ...
        out = torch.cat(out_pyramid, dim=-1)  # (bs, h, w, 81) * 4
        return out.permute(0, 3, 1, 2).contiguous().float()  # (bs, 324, h, w)
