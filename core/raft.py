...
class RAFT(nn.Module):
    ...
    def upsample_flow(self, flow, mask):
        ...
        up_flow = up_flow.view(N, 2, 9, 1, 1, H, W)  # (b,2*3*3,h*w) -> (b,2(u, v 两个方向),9(当前点周围9个光流),1,1,h,w)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True, test_mode=False):
        ...
        # 初始化光流的坐标信息，coords0 为初始时刻的坐标，coords1 为当前迭代的坐标，此处两坐标数值相等  coords1 - coords0
        ...
        for itr in range(iters):
            ...
            corr = corr_fn(coords1)  # index correlation volume  扣出特征图  # (bs, 324, h/8, w/8)

            flow = coords1 - coords0
            with autocast(enabled=self.args.mixed_precision):
                """
                input:
                    net: (bs, 128, h/8, w/8)  左图语义特征 y ht-1 隐状态
                    inp: (bs, 128, h/8, w/8)  左图语义特征 xt
                    corr: (bs, 324, h/8, w/8) 右图多层级关联特征
                    flow: (bs, 2, h/8, w/8)   光流初始值 默认全零
                output:
                    net: (bs, 128, h/8, w/8) 隐状态
                    up_mask: (bs, 64*9, h/8, w/8)   mask
                    delta_flow: (bs, 2, h/8, w/8)   光流变化量
                """
